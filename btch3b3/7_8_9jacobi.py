...
def normalize(A: np.ndarray, b: np.ndarray):
    """ Chuẩn hóa ma trận A để các phần tử trên đường chéo chính là 1. """
    n = len(A)
    for i in range(n):
        div = A[i][i]
...
def jacobi_iteration(A: np.ndarray, b: np.ndarray, max_iter=1000, tol=1e-5):
    """ Phương pháp lặp Jacobi để giải hệ phương trình Ax = b. """
    n = len(A)
    ...
    for _ in range(max_iter):
        x_new = np.copy(x)
        for i in range(n):
            s = np.dot(A[i], x) - A[i][i] * x[i]  # Tổng phần tử hàng i trừ đi phần tử trên đường chéo
            x_new[i] = (b[i] - s) / A[i][i]  # Cập nhật x[i] theo công thức Jacobi
...
if __name__ == "__main__":
    ...
    print("Bài tập 7 - Nghiệm x:", jacobi_iteration(A7, b7))
    ...
    print("Bài tập 8 - Nghiệm x:", jacobi_iteration(A8, b8))
    ...
    print("Bài tập 9 - Nghiệm x:", jacobi_iteration(A9, b9))
